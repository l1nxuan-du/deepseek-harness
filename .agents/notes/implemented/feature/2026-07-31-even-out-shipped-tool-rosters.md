...
`glob`, `grep`, and `rg` are asserted as fixed members rather than host-dependent ones: `dsh-tool-fs-search` spawns the packaged ripgrep binary and registers all three tools unconditionally, so the set is always present.
...
The same model gets the same tools on both surfaces, and the difference that existed for no recorded reason is gone. The tests assert the twenty unconditional names exactly and pin `glob`, `grep`, and `rg` as fixed members on both sides, so a later change that alters only one surface fails a check instead of shipping quietly; the [session-search-not-shipped-default decision](../../archived/feature/2026-08-02-session-search-not-shipped-default.md) is exactly such a later change, and both tests moved with it.
