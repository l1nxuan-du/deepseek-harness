...
Web Host 以当前操作系统用户的权限运行具有工具能力的 Session，但其 HTTP 接口用请求路由事实识别特权调用者。具体而言，按方法维护的 loopback 列表把 loopback `Host` 值视为本地 authority，尽管 HTTP 客户端可以控制该 header。能够到达服务器的调用者因此可以声明 `localhost`、进入配置方法，再利用模型发现等 Host 侧操作披露存储的凭据。随附 CLI 绑定 loopback 可以限制普通可达性，却不能认证被转发或以其他方式送达该 socket 的请求。随附默认改为绑定所有网络接口后，可达性更不能作为 authority 信号。
...
cookie 是签名且绑定 authority 的 bearer。确定性名称与签名 payload 都包含规范化 hostname 和 port，因此同一 Harness home 可以在不同 Web port 运行而不发生 cookie 冲突。payload 在绝对有效期内携带安全整数形式的签发与过期时间；`cookieMaxAgeDays` 默认为 30。cookie 是 host-only、`Path=/`、`HttpOnly`、`SameSite=Strict`。随附 Web 传输使用明文 HTTP，因此不设置 `Secure`。这里没有 logout 操作或反向代理专用处理。
...
随附 CLI 默认绑定所有 IPv4 网络接口，并接受 `--host 127.0.0.1` 将监听限制在本机；该姿态由[默认 Web 绑定](2026-09-18-default-web-bind-all-interfaces.zh.md)负责。认证不代表 TLS、转发 header 解释或代理配置。
...
**增加 logout、TLS 代理和转发 header 配置。** 否决：应用不携带 TLS，也没有反向代理专用约定。网络部署在反向代理处终止 TLS，并通过 `--trusted-host` 声明其 authority；加入 logout 或信任转发 header 会在没有当前 consumer 时定义另一套产品约定。浏览器站点数据控制会撤销单个浏览器会话；删除凭据记录并重启进程会撤销全部会话。
...
持久密钥使 cookie 跨重启生效，也让被盗 cookie 最多保有配置的绝对有效期。删除记录并重启进程是全局撤销机制；当前 Connection 刻意避免在每个请求上访问凭据提供方。由于默认所有网络接口传输使用 HTTP，不设置 `Secure` 会让 cookie 以明文传输。启动 URL 含进程凭据，必须视为敏感输出；运行时诊断不会重复它。
