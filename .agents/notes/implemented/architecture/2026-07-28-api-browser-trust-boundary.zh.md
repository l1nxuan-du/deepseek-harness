...
Web GUI 宿主以纯 HTTP 提供 `/api`（默认 `0.0.0.0:3080`；`--host 127.0.0.1` 可将其限制为 loopback，该姿态由[默认 Web 绑定](2026-09-18-default-web-bind-all-interfaces.zh.md)负责），而这个面上有远程代码执行级别的方法——`session.prompt` 驱动的 agent（智能体）可以运行 bash。浏览器会用两种经典方式把操作者变成攻击此类本地 API 的「混淆代理人」：恶意页面发出跨站「简单请求」 POST（`text/plain`——不经 CORS 预检即发出），其副作用照常执行、只是响应不可读；以及 DNS rebinding 后的源以「同源」身份直连 socket，CORS 整体失效，只有 `Host` 头会暴露攻击者的域名。在本决策之前，系统里唯一的浏览器信任检查（`isTrustedNativeDialogRequest`：回环 socket、同源、回环 Host）只守着一个装饰性的路由——`host.pickDirectory`，其原生对话框弹在宿主屏幕上——而所有真正具有严重后果的方法都没有防护。按 RPC 逐个设防也活不过应用内目录浏览器：它存在的意义就是服务合法的远程客户端，回环规则恰恰会拒绝它们。
...
可达性由 webserver 的绑定配置（`host: 127.0.0.1 | 0.0.0.0`）控制，这道栅栏是混淆代理人防御，而不是身份。随附 CLI 默认绑定所有 IPv4 接口，并支持显式改用 loopback；该姿态由[默认 Web 绑定](2026-09-18-default-web-bind-all-interfaces.zh.md)负责。Connection 在栅栏之后应用独立的[浏览器令牌认证](2026-08-24-browser-token-authentication.zh.md)。栅栏不检查对端 socket 地址：绑定表达可达性，`trustedHosts` 点名接受的 authority，socket 地址提供不了 Host/Origin 校验需要的额外信息。
...
- 自定义非 loopback 组合必须信任其服务 authority，否则请求会被拒绝；随后仍像每个 loopback 请求一样满足浏览器认证。随附 CLI 默认绑定所有 IPv4 接口，并接受 `--host 127.0.0.1` 将监听限制在本机；`--trusted-host` 只扩展 Host/Origin 栅栏，绝不授予身份。
