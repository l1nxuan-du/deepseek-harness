...
The shipped CLI binds all IPv4 interfaces by default and accepts `--host 127.0.0.1` to restrict it; the [default Web bind](2026-09-18-default-web-bind-all-interfaces.md) owns that posture. Authentication does not imply TLS, forwarding-header interpretation, or proxy configuration.
...
**Add logout, TLS-proxy, and forwarding-header configuration.** Rejected: the application carries no TLS or proxy-specific contract. A network deployment terminates TLS at a reverse proxy and names its authority through `--trusted-host`; adding logout or trusting forwarded headers would define separate product contracts without a current consumer. Browser site-data controls revoke one browser session; deleting the credential record and restarting the process revokes all sessions.
