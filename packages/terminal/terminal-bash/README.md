...
One backend serves both dialects: bash and pwsh share the same session machinery — sanitizer, bounded buffers, readiness polling, cancellation, and teardown — and differ only in argv, environment, and prompt installation. Bash receives a private marker through `PS1` plus `PROMPT_COMMAND`. Pwsh writes a prompt function, pins UTF-8 console encoding, and publishes startup only after the backend reports `stdin_read`; echoed setup text cannot publish the shell. That same setup line turns PSReadLine's inline prediction and history saving off, because rendering a predicted history entry crashes PSReadLine 2.4.5 (`IndexOutOfRangeException` from `ConvertOffsetToPoint`) and the crashed reader drops the character it was inserting, which truncates the line the caller submitted; keeping the prediction source quiet also keeps submitted lines out of the console host's shared `ConsoleHost_history.txt`. A zero-scrollback `@xterm/headless` instance consumes raw PTY data and returns terminal-protocol replies through the same handle, while the line sanitizer remains the only output projection.
