...
一个后端服务两种方言：bash 与 pwsh 共享同一套会话机制——清理器、有界缓冲区、就绪轮询、取消与关闭——只在 argv、环境与提示符安装方式上不同。bash 通过 `PS1` 加 `PROMPT_COMMAND` 接收私有标记。pwsh 会写入提示符函数、固定 UTF-8 控制台编码，并只在后端报告 `stdin_read` 后发布启动；回显的设置文本不能发布 shell。同一条设置行还会关闭 PSReadLine 的行内预测与历史保存：渲染一条被预测的历史条目会让 PSReadLine 2.4.5 崩溃（`ConvertOffsetToPoint` 抛出 `IndexOutOfRangeException`），而崩溃的读取器会丢弃当时正在插入的字符，使调用方提交的那一行被截断；让预测源保持安静也避免提交行堆积进控制台宿主共享的 `ConsoleHost_history.txt`。一个不保留 scrollback 的 `@xterm/headless` 实例会消费原始 PTY 数据，并通过同一句柄返回终端协议响应；逐行 sanitizer 仍是唯一输出投影。
