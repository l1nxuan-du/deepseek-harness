...
`host` accepts exactly two values: `127.0.0.1` (loopback only) and `0.0.0.0` (all IPv4 interfaces — deliberate network exposure, since the server carries no TLS, authentication, or origin policy of its own). `port` 0 requests an OS-assigned port; `ctx.webServer.port` reads the listening port afterwards.
