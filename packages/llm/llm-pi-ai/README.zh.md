...
每个 profile 都可以设置 `retryPolicy`；省略时使用 normal mode、最多重试五次。`apiKeyEnv` 是按请求经 harness 凭据 seam 解析的凭据引用，因此配置文件绝不包含密钥；解析为空的引用会让请求以 `MISSING_CREDENTIAL` 失败。省略它会让路由保持已配置但无密钥（configured-but-keyless）状态，对已安装目录路由而言即交由 pi-ai 提供方原生的环境发现。OpenCode 与 OpenCode Go 请求会把会话 id 同时放入 `x-deepseek-harness-session-id` 与 `x-opencode-session`；其他路由仍使用 pi-ai 的提供方专属会话亲和机制。
...
| `api` | 目录协议 | 路由级协议覆盖；在混合协议目录中添加未描述模型时需要 |
| `baseURL` | 目录端点 | 路由级端点覆盖；未描述模型在选定协议后，可继承该协议的目录同类模型唯一共享的端点 |
...
pi-ai 不提供的路由需要 `api`、`baseURL` 与非空 `models` 列表。目录路由中的未描述模型可以先继承唯一的目录协议，再继承该协议下目录同类模型唯一共享的端点；否则路由必须同时提供这两个字段。无法服务的 profile 会在写入处被拒绝，并点名路由与模型。失败携带稳定 code：无法使用的凭据以 `INVALID_CREDENTIAL` 失败并点名路由与引用，`apiKeyEnv` 引用解析为空的路由以 `MISSING_CREDENTIAL` 失败，未配置模型以 `UNKNOWN_MODEL` 失败，终止性提供方失败则区分 `QUOTA` 与暂时性 `RATE_LIMIT`。`GenerateOptions.stop` 以 `UNSUPPORTED_OPTION` 被拒绝，因为 pi-ai 的通用流式 UI 无法跨提供方保证它。
...
- **路由级协议会作用于整条路由**——为混合协议目录路由命名 `api` 可以添加未描述模型，但会覆盖该路由上的所有模型；需要同时使用多种协议时，必须拆成不同路由键。
