...
Each profile may set a `retryPolicy`; omission uses normal mode with five retries. `apiKeyEnv` is a credential reference resolved per request through the harness credential seam, so no secret enters the configuration file; a reference that resolves to nothing fails the request with `MISSING_CREDENTIAL`. Omitting it leaves the route configured-but-keyless, which for an installed catalog route defers to pi-ai's provider-native ambient discovery. OpenCode and OpenCode Go requests carry the Session id in both `x-deepseek-harness-session-id` and `x-opencode-session`; other routes keep pi-ai's provider-specific session-affinity behavior.
