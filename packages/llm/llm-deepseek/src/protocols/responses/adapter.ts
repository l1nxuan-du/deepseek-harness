/**
 * `ResponsesAdapter`: fetch + SSE against a DeepSeek (OpenAI-compatible)
 * Responses endpoint, emitting harness StreamChunks. The adapter is
 * transport-only: connection facts arrive through a thunk resolved once per
 * operation and the bearer token through a per-request resolver, so the
 * registering plugin owns validation, layering, and credential policy.
 *
 * @module dsh-llm-deepseek/protocols/responses/adapter
 */
...
import { serializeResponsesRequest, serializeResponsesRequestWithImages } from './serialize.ts'
...
import type { WireError, WireResponsesRequest } from './types.ts'
...
export class DeepSeekResponsesAdapter extends LlmAdapter {
  ...
  private async * request(
    ...
    while (true) {
      ...
      let body: WireResponsesRequest
      if (attachments === undefined) {
        body = serializeResponsesRequest(requestOptions, connection.defaults)
      } else if (representation === 'base64') {
        body = await serializeResponsesRequestWithImages(requestOptions, {
      ...
      yield* translateResponses(parseSseFrames(response.body, onActivity))
      return
    }
  }
}
