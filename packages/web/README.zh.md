...
`web/` 包让模型通过 `web_search` 与 `web_fetch` 工具搜索公共 web 和抓取 HTTP(S) 页面。部署可使用默认的 Google 主路径与 Bing 回退，也可为搜索选择 Exa、Perplexity 或 DeepSeek，并通过匿名 HTTP(S) 访问抓取页面；可用性与资源上限取决于配置的提供方。该家族用于搜索和页面检索，不用于交互式浏览、内容提取或逐 URL 策略执行。提供方变化时，模型仍能获得一致的工具行为、取消与错误报告。
...
| [`web-search-google-bing/`](web-search-google-bing/README.zh.md) | 先搜索 Google，失败时自动回退 Bing | 注册到 `ctx.web` |
