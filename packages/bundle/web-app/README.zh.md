...
运行 `dsh --profile web`，打开提供聊天、模型与设置管理以及会话历史的交互式浏览器 GUI。它使用与其他 dsh 表层相同的模型访问、工具与安全默认值。启动时会打印带认证信息的 URL，通常还会在默认浏览器中打开；SSH 会话和 `--no-open` 会保留该 URL，供你手动打开。默认监听所有 IPv4 网络接口；使用 `--host 127.0.0.1` 可将其限制在本机。你也可以更改端口并允许额外主机。需要在浏览器中交互式工作时选择本包；一次性的命令行任务应使用 `dsh-headless`。
...
默认情况下 GUI 监听所有 IPv4 网络接口，因此 LAN 内的浏览器可以访问，打印的 URL 会附带一个 LAN 地址。使用 `--host 127.0.0.1` 的部署则只接受本机连接；`--trusted-host` 在两种情况下都能添加额外主机。Host 与 Origin 检查控制可达性，token 交换则认证每个 Host API 方法与 WebSocket 流。LAN 地址只在启动时采样一次，因此之后的网络变化不会被感知——重启 GUI 以重新公告。
...
- **默认网络绑定使用明文 HTTP**——服务器不携带 TLS，其浏览器 cookie 也未标记 `Secure`。仅限本机访问时使用 `--host 127.0.0.1`；需要网络访问时，请在前面部署 TLS 反向代理。
