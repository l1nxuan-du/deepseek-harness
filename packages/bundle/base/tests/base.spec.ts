...
describe('dsh-base bundle', () => {
  it('declares a parseable patch list through the dsh.bundle.patch manifest field', () => {
    ...
    expect(rows.find(row => row.id === 'computer-use')).toBeDefined()
    expect(rows.find(row => row.id === 'computer-use-cua-driver-native')).toBeDefined()
    expect(rows.find(row => row.id === 'web')?.config).toMatchObject({
      fetchProvider: 'http',
      searchProvider: 'google-bing',
    })
    expect(rows.find(row => row.id === 'web-search-google-bing')).toBeDefined()
    ...
    expect(manifest.dependencies).toHaveProperty('@deepseek-ai/dsh-web-search-google-bing')
    expect(manifest.dependencies).toHaveProperty('@deepseek-ai/dsh-computer-use')
    expect(manifest.dependencies).toHaveProperty('@deepseek-ai/dsh-experimental-computer-use-cua-driver-native')
  })
