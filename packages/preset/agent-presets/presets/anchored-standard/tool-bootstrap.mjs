...
/**
 * The default first-request catalog: the Minimal condition's tool pair — the
 * platform's own persistent shell plus `str_replace_editor`. The upstream
 * preset measured this pair anchoring 5/5 at the adapter-default maxTokens
 * while every standard-family schema failed 11/11; the Windows member is this
 * harness's persistent PowerShell rather than the POSIX shell, because that is
 * the shell `minimal` mounts here.
 */
const DEFAULT_BOOTSTRAP_TOOLS = process.platform === 'win32'
