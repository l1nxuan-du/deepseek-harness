...
- **浏览器 cookie 不带 `Secure`**：随附 Web 传输使用明文 HTTP 且默认绑定所有网络接口，因此网络观察者可能在传输中截获 bearer cookie。仅在可信网络中使用，或改用 `--host 127.0.0.1` / TLS 反向代理。
