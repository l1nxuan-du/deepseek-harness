...
- **The browser cookie is not marked `Secure`** — the shipped Web transport is plain HTTP and binds all interfaces by default, so a network observer can capture the bearer cookie in transit. Use `--host 127.0.0.1` or a TLS reverse proxy on a trusted network.
