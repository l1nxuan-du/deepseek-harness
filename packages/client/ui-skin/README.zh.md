...
- 背景场：一个前置到 body 的固定定位元素，包含渐变层、WebGL2 流动图案画布，以及指针光晕。
...
背景场是普通 DOM 而非 React：一个固定元素，包含渐变层、运行流动图案的 WebGL2 画布，以及用 CSS 表达的指针光晕。指针位置合并到同一动画帧，落到两个自定义属性上；光晕是样式表按属性切换的径向渐变。背景场不承载任何产品状态。
...
- **有意去掉格网**——设计稿中的 90px 网格不再保留；背景场由渐变、流动图案与指针光晕构成。
