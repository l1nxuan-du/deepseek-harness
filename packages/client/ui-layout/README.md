...
Windows Electron's `data-windows-titlebar` marker reserves the caption height above all columns and removes the collapsed sidebar rail. With that rail gone the conversation keeps the material pane inset on its left edge, so it does not sit flush against the window; ordinary Web keeps the rail and receives no such margin. The caption padding survives the material skin, whose runtime-injected sheet pads the frame from the pane inset. That sheet states the inset with a padding shorthand whose selector outranks the frame rule, so the frame's padding-top carries !important. Only the content area's top-left corner has a 16px radius; the other corners and the internal divider remain square. The frame publishes `--dsh-windows-content-radius` and `--dsh-windows-sidebar-width` for ui-sidebar-right's fullscreen corner and sidebar clearance. Ordinary Web documents do not receive the marker; macOS retains its separate layout.
