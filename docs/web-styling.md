...
[`ui-skin`](../packages/client/ui-skin/README.md) owns the user's chrome choice: the material chrome the chat study describes (the product default), or the shipped `classic` chrome. Its material sheet is global but every rule requires `html[data-dsh-skin='material']`, which the skin runtime sets only while that chrome is selected, so an install cannot restyle the classic chrome. Palette axes for the material chrome ride the alias-token layer through `ctx.theme.overrideTokens`; the sheet owns structure (pane insets and radii, backdrop blur, bubbles, the composer card, the tab indicator) and reaches the shipped components through their slot seats plus stable class-name suffixes. A control that both chromes share still belongs in [ui-primitives](../packages/client/ui-primitives/README.md) as a prop, and a new shared token still belongs to ui-theme.
