...
The production Web runner needs built package and frontend artifacts (`pnpm run build`). It listens on all IPv4 interfaces on port 3080 by default; the canonical host URL is `http://127.0.0.1:3080`, and a local launch opens that URL only after the complete Loader tree settles. Pass `--host 127.0.0.1` to restrict the listener to this machine. The default bind is plain HTTP; use a trusted network or a TLS reverse proxy. A non-empty inherited `SSH_CONNECTION` or `SSH_TTY` suppresses the browser handoff because the SSH client or editor owns the local forwarded address; the host URL is still printed. Immediately before a local handoff it prints `dsh web: opening the default browser; pass --no-open to disable`; if the operating-system handoff fails, a diagnostic on stderr states the reason, leaves the server running, and names the URL for manual use. `--trusted-host` adds named authorities accepted by the `/api` browser-trust fence.
