...
The Windows installer shows the stock NSIS pages the uninstaller already uses: a welcome page with the product sidebar, a directory page that starts at the machine-wide default, one plain options page for the desktop-shortcut and Start-menu choices (both selected by default), the installation progress page, and a finish page whose launch checkbox is selected by default. The chosen folder must be a full local path outside drive roots, system folders, links, and special characters, it must be empty unless it is the registered installation directory, and a folder whose name is not the product folder receives that folder below it. Installation is machine-wide under `%ProgramFiles%`, so Windows requests elevation before the wizard appears. The installer never stops an installation itself: while the executable at the registered installation path runs, an update waits up to ten seconds for it to exit, then stops with exit code 2, and the affected directory stays in place.

A silent installation (`/S`) skips the pages, keeps both shortcuts, and applies the same destination rules to `/D=`. Directory replacement and failure recovery follow the installation flow described above, and the finish page starts the installed executable unless its Run checkbox is cleared. First-launch profile preparation remains a separate Desktop operation.

Windows packaging rasterizes the wizard sidebar into the 164×314 BMP both wizards embed, and the preparation hook leaves production dependency collection to electron-builder on every platform.

Run `pnpm --dir apps/desktop run test:installer` from the repository root in an elevated Windows x64 desktop session, because the shipped installer and its shortcuts write machine-wide state. The command builds a small native test payload through the production installer configuration and sequentially exercises English-only and Chinese-only variants, each of which installs into a private directory and uninstalls after testing. It clicks the welcome, options, progress, and finish pages, checks both shortcut choices and the launch option, refuses a non-empty destination the installation does not own, updates a registered directory stored with trailing separators, blocks replacement while the application runs, and records screenshots and results under `.desktop-build/installer-tests/`. The optional `--signed` flag uses the Windows EV configuration below to sign test executables before embedding them; it does not enable an update feed.
