...
The Windows installer shows the stock NSIS pages the uninstaller already uses: a welcome page with the product sidebar, one options page for the desktop-shortcut and Start-menu choices (both selected by default), the installation progress page, and a finish page whose launch checkbox is selected by default. Installation is machine-wide under `%ProgramFiles%`, so Windows requests elevation before the wizard appears. The installer never stops an installation itself: while the executable at the registered installation path runs, an update waits up to ten seconds for it to exit, then stops with exit code 2, and the affected directory stays in place.

A silent installation (`/S`) skips the pages, keeps both shortcuts, and validates its destination before writing: `/D=` must name a full local path outside drive roots, system folders, links, and special characters, and that path must be empty unless it is the registered installation directory. Directory replacement and failure recovery follow the installation flow described above, and the finish page starts the installed executable unless its Run checkbox is cleared. First-launch profile preparation remains a separate Desktop operation.
