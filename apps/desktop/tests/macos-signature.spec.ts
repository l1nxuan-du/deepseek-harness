...
describe('desktop macOS release signature', () => {
  ...
  it('loads release identifiers from the environment and requires code signing', async () => {
    ...
    const extraResources = config.extraResources as ReadonlyArray<{ readonly from?: string; readonly to?: string }>
    expect(extraResources).toHaveLength(3)
    expect(portablePath(extraResources[1]?.from ?? '')).toContain('/resources/icon-windows.png')
    expect(extraResources[2]).toMatchObject({ to: 'icon-tray.png' })
