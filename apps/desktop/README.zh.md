...
桌面应用是完整 dsh Web 应用外的一层 Electron 壳。Electron RunAsNode 子进程启动共享 profile runner，Electron 立即从 `dsh-app://app/` 加载打包内的 Web 入口。共享加载页等待 Host 启动注入，然后在同一文档中启动客户端。Electron 将应用 HTTP 请求转发给已认证的 Web Host；WebSocket 流连接到该 Host，仅为归属的应用窗口附加凭据。Node IPC 承载启动注入、就绪与关闭。Desktop 绑定 `127.0.0.1:19387`，与 Web 默认绑定所有网络接口的 `3080` 分开；可通过 `webserver` patch 覆盖端口。

应用菜单第一项“**关于 DeepSeek Harness**”打开 Electron 原生关于面板，展示应用图标、产品名称和当前安装的发布版本。菜单文案跟随桌面壳的语言。macOS 从应用包读取图标，因此未打包的开发启动会显示 Electron 图标；Windows 使用随包分发的 PNG。通知区图标让应用保持可访问：其菜单提供“打开界面”和“退出”，关闭应用窗口时窗口隐藏到该图标，而不是结束进程。
...
Windows 安装程序沿用卸载程序使用的原生 NSIS 页面：带产品侧栏的欢迎页、以整机默认目录开始的安装位置页、承载桌面快捷方式与开始菜单选项（默认均勾选）的一个选项页、安装进度页，以及默认勾选立即启动的完成页。所选目录必须是完整的本地路径，不能是磁盘根目录、系统目录、链接目录或包含特殊字符的路径；它必须为空，除非它是已登记的安装目录；若目录名不是产品目录名，安装程序会在其下补上该产品目录。安装面向整机、位于 `%ProgramFiles%` 之下，因此 Windows 会在向导出现前请求提升权限。安装程序不会自行结束应用：当已登记安装目录中的可执行文件正在运行时，更新最多等待十秒，随后以退出码 2 结束，原目录保持不变。

静默安装（`/S`）跳过页面、保留两个快捷方式，并对 `/D=` 应用相同的目标目录规则。目录替换和失败恢复遵循上文描述的安装流程；完成页在未取消勾选“立即启动”时启动已安装的可执行文件。首次启动的配置档案准备仍属于独立的 Desktop 操作。

Windows 打包将向导侧栏转换为两个向导共用的 164×314 BMP；准备钩子在所有平台上均由 electron-builder 继续负责收集生产依赖。

在具备交互式桌面的 Windows x64 上，需以提升权限的会话从仓库根目录运行 `pnpm --dir apps/desktop run test:installer`，因为正式安装程序及其快捷方式会写入整机状态。该命令将小型原生测试载荷接入正式安装配置，依次验证仅英文和仅中文的变体；每个变体安装到私有目录并在测试后卸载。检查会依次点击欢迎页、选项页、进度页和完成页，核对两个快捷方式选项与启动选项，拒绝不属于本安装的非空目标目录，升级以末尾带分隔符登记的安装目录，在应用运行期间阻止替换，并把截图与结果保存在 `.desktop-build/installer-tests/` 下。可选的 `--signed` 标志使用下文的 Windows EV 配置在嵌入前对测试程序签名；它不会启用更新源。
